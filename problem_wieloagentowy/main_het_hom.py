...
def main():
    ...
    try:
        ...
        from trainer_het_hom import EnhancedMultiAgentTrainer
    ...
    print("\n🔧 Konfiguracja eksperymentu:")
    config = {
        'n_players': 3,              # 4 graczy w Texas Hold'em
        'total_timesteps': 1000,    # Zwiększona liczba kroków dla lepszej jakości
        'eval_freq': 500,           # Częstotliwość ewaluacji
        'n_eval_episodes': 2,       # Więcej epizodów dla stabilniejszych wyników
        'max_combinations': 2        # Maksymalna liczba kombinacji heterogenicznych
    }
    ...
    all_algorithms  = AlgorithmsConfig.get_algorithms()
    fast_algorithms = {}
    algorithm_names = list(all_algorithms.keys())

    # Priorytet: A2C i PPO (zwykle najszybsze)
    priority_algorithms = ['A2C', 'PPO']

    for alg_name in priority_algorithms:
        for full_name in algorithm_names:
            if alg_name in full_name:
                fast_algorithms[full_name] = all_algorithms[full_name]
                break

    # Jeśli nie znaleźliśmy wystarczająco, bierzemy pierwsze 2
    if len(fast_algorithms) < 2:
        fast_algorithms = dict(list(all_algorithms.items())[:2])

    print(f"📋 Wybrane algorytmy (szybkie): {list(fast_algorithms.keys())}")
    ...
    trainer.train_homogeneous_scenario(
        algorithm_configs=fast_algorithms,
    ...
    trainer.train_heterogeneous_scenarios(
        algorithm_configs=fast_algorithms,
        total_timesteps=config['total_timesteps'] // 4,
