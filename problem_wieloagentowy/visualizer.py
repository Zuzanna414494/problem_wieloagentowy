...
class ResultsVisualizer:
    ...
    def plot_learning_curves(self):
        """Krzywe uczenia (jeśli dostępne dane 'episode_rewards')"""
        ...
        for alg_name, data in self.results.items():
            if 'episode_rewards' in data and data['episode_rewards']:
                ...
                ax.plot(smoothed, label=alg_name, color=self.colors.get(alg_name, '#333'), linewidth=2)
                has_data = True

        if has_data:
            ax.set_title('Krzywe uczenia algorytmów', fontsize=16, fontweight='bold')
            ax.set_xlabel('Epizody')
            ax.set_ylabel('Średnia nagroda (wygładzona)')
            ax.legend()
            ax.grid(True, linestyle='--', alpha=0.5)
            plt.tight_layout()
            plt.savefig(self.output_dir / 'learning_curves.png', dpi=300)
            plt.close()
            print("✅ Wykres krzywych uczenia zapisany")
        else:
            print("⚠️ Brak danych 'episode_rewards' do wykresu uczenia")
