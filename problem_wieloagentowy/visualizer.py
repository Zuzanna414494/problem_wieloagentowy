...
class ResultsVisualizer:
    ...
    def __init__(self, results: Dict[str, Dict[str, Any]]):
        ...
        # Konfiguracja kolorów - dopasowane do algorytmów z algorithms_config.py
        # Algorytmy z algorithms_config.py: PPO_Conservative, PPO_Aggressive, A2C_Standard, DQN_Standard
        self.colors = {
    ...
    def create_all_plots(self):
        """Tworzy wszystkie wykresy, z naciskiem na krzywe uczenia."""
        ...
        # Nowy, główny wykres: krzywe uczenia
        self.plot_learning_curves()

        # Pozostałe podstawowe wykresy
    ...
    def plot_learning_curves(self):
        """
        Wykres krzywej uczenia dla każdego algorytmu.
        Wymaga, aby dane 'episode_rewards' były dostępne w słowniku results dla każdego algorytmu.
        """
        ...
        for alg_name, data in self.results.items():
            if 'episode_rewards' in data and data['episode_rewards']:
                ...
                # Użyj rolling mean do wygładzenia krzywej
                window_size = max(1, len(rewards) // 10) # Przykładowo 10% długości danych
                smoothed_rewards = pd.Series(rewards).rolling(window=window_size, min_periods=1).mean()

                ax.plot(smoothed_rewards, label=alg_name, color=self.colors.get(alg_name, '#333333'), linewidth=2)
                has_learning_data = True
            else:
                print(f"⚠️ Brak danych 'episode_rewards' dla algorytmu: {alg_name}. Pomięto wykres uczenia.")

        if has_learning_data:
            ax.set_title('Krzywe uczenia algorytmów (wygładzone)', fontsize=16, fontweight='bold')
            ax.set_xlabel('Epizody / Kroki treningowe', fontsize=12)
            ax.set_ylabel('Średnia nagroda epizodyczna (wygładzona)', fontsize=12)
            ax.legend(title='Algorytm', loc='lower right')
            ax.grid(True, linestyle='--', alpha=0.6)
            plt.tight_layout()
            plt.savefig(self.output_dir / 'learning_curves.png', dpi=300, bbox_inches='tight')
            plt.close()
            print("✅ Wykres krzywych uczenia utworzony")
        else:
            print("❌ Nie można utworzyć wykresu krzywych uczenia: Brak danych 'episode_rewards' dla wszystkich algorytmów.")
    ...
    def save_report_data(self, filepath: str = None):
        ...
        # Save to 'results' directory, sibling to 'plots', consistent with main.py's saving
        if filepath is None:
            report_dir = self.output_dir.parent / 'results'
            report_dir.mkdir(exist_ok=True) # Ensure 'results' directory exists
            filepath = report_dir / 'experiment_report.txt'
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for alg_name, data in self.results.items():
                ...
                # Handle potential division by zero for efficiency calculation
                efficiency = data['mean_reward'] / (data['training_time'] / 60) if data['training_time'] > 0 else 0
                f.write(f"  Efektywność: {efficiency:.4f} nagroda/min\n")
                f.write(f"  Timesteps: {data['total_timesteps']}\n")

        print(f"📝 Dane raportu zapisane do: {filepath}")

# Test modułu (pozostawione dla celów testowych, ale w kontekście projektu będzie wywoływane przez main.py)
if __name__ == "__main__":
    ...
    test_results = {
        'PPO_Conservative': {
            ...
            'std_reward': 0.05,
            'training_time': 300,
            'hyperparams': {'learning_rate': 3e-4, 'gamma': 0.99, 'batch_size': 64, 'ent_coef': 0.01},
        ...
        'PPO_Aggressive': {
            ...
            'std_reward': 0.08,
            'training_time': 250,
            'hyperparams': {'learning_rate': 1e-3, 'gamma': 0.95, 'batch_size': 128, 'ent_coef': 0.05},
            'total_timesteps': 100000,
        },
        'A2C_Standard': {
            'mean_reward': 0.08,
            'std_reward': 0.04,
            'training_time': 200,
            'hyperparams': {'learning_rate': 7e-4, 'gamma': 0.99, 'batch_size': 32, 'ent_coef': 0.01},
            'total_timesteps': 100000,
        },
        'DQN_Standard': {
            'mean_reward': 0.10,
            'std_reward': 0.06,
            'training_time': 280,
            'hyperparams': {'learning_rate': 5e-4, 'gamma': 0.98, 'batch_size': 64, 'train_freq': 4},
            'total_timesteps': 100000,
        }
    }
